	...
	*/
	...
	public function getDefaultConfig($config = null)
	{
		...
		$config[self::CONFIG_FILTER] = [
				"types" => "",
				"parents_childs" => "0",
				"stock_status_dependence" => \Nostress\Koongo\Model\Config\Source\Stockdependence::STOCK,
				"export_out_of_stock" => "0",
				"visibility" => array("4"),
				"visibility_parent" => array("4"),
				"conditions" => ""
		];
		...
		$common = 	[
				...
				"all_images" => "1",
				"all_categories" => "1",
				"shipping" => ["dependent_attribute" => $this->attributeSource->addModuleAttributePrefix("price_final_include_tax")]
					];
		...
		$rule = [	"days_interval" => \Nostress\Koongo\Model\Config\Source\Scheduledays::EVERY_DAY,
        			"times_interval" => \Nostress\Koongo\Model\Config\Source\Scheduletimes::EVERY_24H,
        			"time_hours" => "1",
        			"time_minutes" => "0",
        			"enabled" => "on"];
    ...
    public function getAttributesWithDescription()
    {
    	...
    	foreach($feedAttributes as $feedAttribute)
    	{
    		if(isset($feedAttribute[self::CONFIG_ATTRIBUTE_CODE]))
    			...
    			$code = $this->helper->createCode($attribute[self::CONFIG_ATTRIBUTE_LABEL]);
	...
	* Get parameters for XSL and XML tranformation
    ...
    public function getTransformParams()
    {
    	if(!isset($this->_transformParams))
    	{
    		...
    		$params["store_id"] = $this->getStoreId();
    		$params["profile_id"] = $this->getId();
    		$params["parents_childs"] = $this->getArrayField(self::CONFIG_FILTER_PARENTS_CHILDS,$filterConfig,"0");
    ...
    public function getConfigItem($index,$exception = true,$subIndex = null)
    {
    	$config = $this->getConfig();
    ...
    public function getPreview() {
    	...
    	if( $this->getFeed()->getFileType() == 'xml') {
    		...
    		$preview = $this->profileHelper->readXmlPreview( $url);
    	} else {
    		...
    		$preview = $this->profileHelper->readCsvPreview( $url, $cd, $enc);
    	}
    ...
    */
    protected function getTransformStoreParams($params = array())
    {
    	if(!isset($this->_storeDependentParams))
    	{
	    	...
	    	$mediaUrl = $this->helper->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
	    	...
	    	$format = \Nostress\Koongo\Model\Config\Source\Datetimeformat::STANTDARD;
	...
	protected function getFileUrl()
	{
		...
		$feedDir = $this->getFeedDirectoryName();
		$filename = $this->helper->getFeedStorageUrl($filename,$feedDir);
		return $filename;
	}
