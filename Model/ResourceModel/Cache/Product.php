...
class Product extends \Nostress\Koongo\Model\ResourceModel\Cache
{
    ...
    protected function defineColumns()
    {
        ...
        $this->_columns[self::CPAMG] = array("product_id" => "entity_id", "store_id" => "({$this->getStoreId()})",);
    ...
    protected function insertRecords()
    {
        ...
        $this->runQuery($sql, $this->getMainTable(), "Insert records. Filled columns: product_id, store_id, min_price, price, qty.");
    }
    ...
    public function getCacheColumns($type = null)
    {
        ...
        switch ($type) {
            case"media_gallery":
                ...
                break;
            case"category":
                $columns = array_merge($this->getColumns($this->getCategoryFlatTable(true), null, false, true), $this->getColumns(self::NKCCP, null, false, true));
    ...
    {
        if (isset($status)) $this->_excludedImagesExportEnabled = $status;
    }
    ...
    {
        if (isset($this->_stockWebsiteId)) return $this->_stockWebsiteId; else return $this->getWebsiteId();
    }
    ...
    protected function getInsertRecordsSql()
    {
        ...
        $minPriceCondition = "CASE\n\tWHEN {$catRulePriceAlias}.rule_price IS NULL && {$mainTableAlias}.special_price IS NOT NULL THEN {$mainTableAlias}.special_price\n    WHEN {$catRulePriceAlias}.rule_price IS NOT NULL && {$mainTableAlias}.special_price IS NULL THEN {$catRulePriceAlias}.rule_price\n    WHEN {$catRulePriceAlias}.rule_price < {$mainTableAlias}.special_price THEN {$catRulePriceAlias}.rule_price\n    WHEN {$catRulePriceAlias}.rule_price > {$mainTableAlias}.special_price THEN {$mainTableAlias}.special_price\n    WHEN {$mainTableAlias}.price IS NULL THEN cpip.min_price\n    ELSE {$mainTableAlias}.price\nEND ";
        $columns = array("product_id" => $mainTableAlias . ".entity_id", "store_id" => "({$this->getStoreId()})", "min_price" => "(" . self::MINIMAL_PRICE_CONDITION_SBST_STRING . ")", "price" => "IFNULL({$mainTableAlias}.price, {$catProdIdxPriceAlias}.max_price)", "qty" => $this->helper->getRoundSql("{$catInvStockStatusAlias}.qty", 0));
        $select->from([$mainTableAlias => $mainTable], $columns);
        $select->joinLeft([$catRulePriceAlias => $catRulePrice], "{$mainTableAlias}.entity_id = {$catRulePriceAlias}.product_id AND {$catRulePriceAlias}.rule_date = CURDATE() AND {$catRulePriceAlias}.website_id = {$this->getWebsiteId()} AND {$catRulePriceAlias}.customer_group_id = " . self::DEF_CUSTOMER_GROUP_NOT_LOGGED_IN, null);
        $select->joinLeft([$catProdIdxPriceAlias => $catProdIdxPrice], "{$mainTableAlias}.entity_id = {$catProdIdxPriceAlias}.entity_id AND {$catProdIdxPriceAlias}.website_id = {$this->getWebsiteId()} AND {$catProdIdxPriceAlias}.customer_group_id = " . self::DEF_CUSTOMER_GROUP_NOT_LOGGED_IN, null);
        $select->joinLeft([$catInvStockStatusAlias => $catInvStockStatus], "{$mainTableAlias}.entity_id = {$catInvStockStatusAlias}.product_id AND {$catInvStockStatusAlias}.stock_id = " . self::DEFAULT_STOCK_ID . " AND {$catInvStockStatusAlias}.website_id = {$this->getStockWebsiteId()}", null);
    ...
    protected function getMediaGallerySql()
    {
        ...
        $cpemgvTableAliasValue = "value";
        $cpemgvTableAliasDefaultValue = "defaultValue";
        $columns = array("product_id" => $cpemgvteTableAlias . ".entity_id", "store_id" => "({$this->getStoreId()})", "value" => $cpemgTableAlias . ".value", "label" => "IFNULL(IFNULL({$cpemgvTableAliasValue}.label,{$cpemgvTableAliasDefaultValue}.label),'')");
        $subSelect = $this->getEmptySelect();
        $subSelect->from([$cpemgvteTableAlias => $cpemgvteTable], $columns);
        $subSelect->joinLeft([$cpemgTableAlias => $cpemgTable], "{$cpemgvteTableAlias}.value_id = {$cpemgTableAlias}.value_id AND  {$cpemgTableAlias}.media_type = 'image'", null);
        $subSelect->joinLeft([$cpemgvTableAliasValue => $cpemgvTable], "{$cpemgvTableAliasValue}.value_id= {$cpemgTableAlias}.value_id AND {$cpemgvTableAliasValue}.store_id = {$this->getStoreId()}", null);
        $subSelect->joinLeft([$cpemgvTableAliasDefaultValue => $cpemgvTable], "{$cpemgvTableAliasDefaultValue}.value_id= {$cpemgTableAlias}.value_id AND {$cpemgvTableAliasDefaultValue}.store_id = 0", null);
        $subSelect->order(array("{$cpemgvteTableAlias}.entity_id", $cpemgvTableAliasValue . ".position", $cpemgvTableAliasDefaultValue . ".position",));
        if (!$this->getExcludedImagesExportEnabled()) {
            $subSelect->where("{$cpemgvTableAliasValue}.disabled = ?", 0);
            $subSelect->orWhere("{$cpemgvTableAliasValue}.disabled IS NULL AND {$cpemgvTableAliasDefaultValue}.disabled = 0");
        }
        $select = $this->getEmptySelect();
        $select->from(array("concat_media_gallery" => $this->getSubSelectTable($subSelect)), ["product_id", "store_id"]);
    ...
    protected function getCategoryMaxLevelSql()
    {
        ...
        $select->joinLeft([$catProdTableAlias => $catProdTable], $catProdTableAlias . ".product_id=" . $mainTableAlias . ".entity_id ", null);
        $select->joinLeft([$catTableAlias => $catTable], $catProdTableAlias . ".category_id=" . $catTableAlias . ".entity_id", ["max_level" => "MAX(level)"]);
    ...
    protected function getProductCategoriesSql()
    {
        ...
        $select->join(array($joinTableAlias => $joinTable), $joinTableAlias . ".entity_id=" . $mainTableAlias . ".category_id", null);
    ...
    protected function getConfigurableQtySql()
    {
        ...
        $select->join(array($cprTableAlias => $cprTable), "{$cprTableAlias}.child_id = {$cissTableAlias}.product_id AND {$cprTableAlias}.parent_id IS NOT NULL", null);
        $select->join(array($cpfTableAlias => $cpfTable), "{$cpfTableAlias}.entity_id = {$cprTableAlias}.parent_id AND {$cpfTableAlias}.type_id = 'configurable'", null);
